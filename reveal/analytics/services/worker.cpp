...
void worker_c::work( void ) {
  ...
  while( true ) {
    ...
    /// NOTE: not sure if anything is being received at the port
    ...
    if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_HANDSHAKE ) {
      Reveal::Core::authorization_ptr auth = exchange.get_authorization();

      service_handshake_request( auth );
      // TODO : error checking

    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_REQUEST ) {

    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_RESPONSE ) {

    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_COMMAND ) {
/*
    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_ERROR ) {
*/
    } 
...
//-----------------------------------------------------------------------------
bool worker_c::authorize( Reveal::Core::authorization_ptr auth ) {
  assert( auth );
  return true;
}

//-----------------------------------------------------------------------------
bool worker_c::service_failed_authorization( Reveal::Core::authorization_ptr auth ) {
  Reveal::Analytics::exchange_c exchange;
  std::string reply;
  ...
  exchange.set_origin( Reveal::Analytics::exchange_c::ORIGIN_ADMIN );
  exchange.set_type( Reveal::Analytics::exchange_c::TYPE_ERROR );
  exchange.set_error( Reveal::Analytics::exchange_c::ERROR_AUTHORIZATION );
...
//-----------------------------------------------------------------------------
bool worker_c::send_valid_handshake_response( Reveal::Core::authorization_ptr auth ) { 
  Reveal::Analytics::exchange_c exchange;
  std::string reply;

  // construct the return handshake message
  exchange.set_origin( Reveal::Analytics::exchange_c::ORIGIN_ADMIN );
  exchange.set_type( Reveal::Analytics::exchange_c::TYPE_HANDSHAKE );
...
//-----------------------------------------------------------------------------
bool worker_c::send_invalid_handshake_response( Reveal::Core::authorization_ptr auth ) { 
  Reveal::Analytics::exchange_c exchange;
  std::string reply;
...
//-----------------------------------------------------------------------------
bool worker_c::service_handshake_request( Reveal::Core::authorization_ptr auth ) { 
...
void worker_c::work( void ) {
  ...
  while( true ) {
    ...
    // parse the serialized request
    if( exchange.parse( request ) != Reveal::Analytics::exchange_c::ERROR_NONE ) {
      printf( "ERROR: Failed to parse client request\n" );
      // TODO: improve error handling.  Bomb or recover here.       
    }
    ...
    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_DIGEST ) {
      Reveal::Core::authorization_ptr auth = exchange.get_authorization();
    ...
    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_EXPERIMENT ) {
      Reveal::Core::authorization_ptr auth = exchange.get_authorization();
    ...
    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_TRIAL ) {
      Reveal::Core::authorization_ptr auth = exchange.get_authorization();
      if( authorize( auth ) == ERROR_NONE ) {
    ...
    } else if( exchange.get_type() == Reveal::Analytics::exchange_c::TYPE_SOLUTION ) {
      Reveal::Core::authorization_ptr auth = exchange.get_authorization();
...
worker_c::error_e worker_c::service_digest_request( Reveal::Core::authorization_ptr auth ) {
  ...
  if( db_error == Reveal::DB::database_c::ERROR_NONE ) {
    ...
    // construct the digest message
    exchange.set_origin( Reveal::Analytics::exchange_c::ORIGIN_SERVER );
    exchange.set_type( Reveal::Analytics::exchange_c::TYPE_DIGEST );
...
worker_c::error_e worker_c::service_experiment_request( Reveal::Core::authorization_ptr auth, std::string scenario_id, Reveal::Core::experiment_ptr experiment ) {
  ...
  exchange.set_origin( Reveal::Analytics::exchange_c::ORIGIN_SERVER );
  exchange.set_type( Reveal::Analytics::exchange_c::TYPE_EXPERIMENT );
...
worker_c::error_e worker_c::service_trial_request( Reveal::Core::authorization_ptr auth, Reveal::Core::experiment_ptr experiment, double t, double epsilon ) {
  ...
  if( db_error == Reveal::DB::database_c::ERROR_NONE ) {
    ...
    exchange.set_origin( Reveal::Analytics::exchange_c::ORIGIN_SERVER );
    exchange.set_type( Reveal::Analytics::exchange_c::TYPE_TRIAL );
  ...
  if( db_error == Reveal::DB::database_c::ERROR_EMPTYSET ) {
    ...
    exchange.set_origin( Reveal::Analytics::exchange_c::ORIGIN_SERVER );
    exchange.set_type( Reveal::Analytics::exchange_c::TYPE_ERROR );
    exchange.set_error( Reveal::Analytics::exchange_c::ERROR_BAD_TRIAL_REQUEST );
    exchange.set_authorization( auth );
...
//  printf( "solution submitted\n" );
  ...
  exchange.set_origin( Reveal::Analytics::exchange_c::ORIGIN_SERVER );
  exchange.set_type( Reveal::Analytics::exchange_c::TYPE_SOLUTION );
  exchange.set_authorization( auth );
  exchange.set_experiment( experiment );
