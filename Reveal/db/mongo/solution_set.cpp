...
//-----------------------------------------------------------------------------
bool solution_set_c::fetch( Reveal::Core::solution_set_ptr& solution_set, Reveal::DB::database_ptr db, std::string experiment_id, unsigned trial_id ) {
  ...
  result = trial_c::fetch( trial, db, scenario_id, trial_id );
  if( !result ) return false;
  solution_set->add_trial( trial );

  result = solution_c::fetch( solution, db, experiment->experiment_id, scenario_id, trial_id );
  if( !result ) return false;
  solution_set->add_solution( solution );

  result = solution_c::fetch( model, db, Reveal::Core::solution_c::MODEL, scenario_id, trial_id );
  if( !result ) return false;
  solution_set->add_model( model );
