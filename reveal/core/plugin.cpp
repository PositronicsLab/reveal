...
*/  
//-----------------------------------------------------------------------------
error_e plugin_c::read( std::string function_name, void* signiture_f, void* function ) {
  assert( HANDLE );

  // locate the function
  function = (void*) dlsym( HANDLE. function_name )
...
error_e plugin_c::read( const char* path ) {
  ...
  // locate a function
  void* f = (void*) dlsym( HANDLE. handle_name )
